...
import sys, gzip, math, time, re, docopt, subprocess
...
def read_wig(sample_file):
    """reads a wig file and returns a dictionary"""
    if sample_file.endswith('.gz'):
        # file = gzip.open(sample_file, 'r')
...
def extract_sample(tsv_path, sample_name):
    ...
    for line in file:
        ...
        if cols[0] != chrom:
            ...
            values.append(float(cols[sampleIdx]))
...
def smooth_allele_fractions(allele_fractions, win_size):
    """Smoothes and mirrors the allele fraction data simultaneously and returns it"""
    for chrom in allele_fractions:
        ...
        heterozygosity = 1 - (2 * abs(np.subtract(0.5, allele_fracs)))
        allele_fractions[chrom].values = heterozygosity * l2 + ((1 - heterozygosity) * l1)
    return allele_fractions
    ...
    for chrom in allele_fractions:
        ...
        while win_start < chr_size:
            ...
            while first_out_snp < len(positions) and positions[first_out_snp] <= win_end:
                ...
                win_count += 1
                first_out_snp += 1
            if win_count < (win_size/2) or win_count < 10:    ##if there is not as much snp as win_size puts nan. 
...
def double_sliding_window_all_chromosomes(log_ratios, win_size):
    """finds the mean differences for the coverage logratios of all the chromosomes"""
    mean_differences = {}
...
def filter_outliers(log_ratios, win_size):
    """filters out outliers in logratios using the median filter and returns the logratios"""
    for chrom in log_ratios:
...
def calculate_logratios(tumor_sample, reference_sample, min_read):
    ...
    modes = [] ##Modes of logratios for each chromosome to take care of the systematic bias
    for chrom in reference_sample:
        ...
        tumor_sample_values[mask] = np.nan
    ...
    return log_ratios     ## Bias free logratios
...
def find_consensus_change_points_win_wise(log_ratios, allele_fractions, win_size, logr_thresh, baf_thresh):
    ...
    start = log_ratio_mean_diffs[log_ratio_mean_diffs.keys()[0]].start
    step = log_ratio_mean_diffs[log_ratio_mean_diffs.keys()[0]].step
    ...
    ##Finding the changepoints for one window size    
    for chrom in log_ratio_mean_diffs:
        ...
        #############################################
        ## Fix for when no changepoint is detected ##
        #############################################
        if len(indices) == 0:
            sys.stderr.write("No break points were detected for %s with win_size: %s; clogr_threshold: %s, and BAF_threshold: %s.\n" %(chrom, win_size, logr_thresh, baf_thresh))
            chr = Chromosome()
...
def consensus(change_points_list, window_sizes, log_ratios):  
    """finds the consensus changepoints"""
    consensus_cp = {}
...
def make_seg(log_ratios, allele_fractions, change_points, out_path, logr_merge_thresh, baf_merge_thresh):    
    """make a SEG file containing the segments based on cLOGr and BAF"""
    id = out_path.split('.')[0]
    file = open(out_path, 'w')
    file.write("'ID \t chrom \t loc.start \t loc.end \t seg.mean \t baf.mean  \n")
    # print "'ID\tchrom\tloc.start\tloc.end\tseg.mean\tbaf.mean"
    ...
    for chrom in change_points:
        ...
        while i < len(cp_indices) - 2:
            ...
            if str(clogr_avg1) == 'nan':
                file.write("%s\t%s\t%d\t%d\t%f\t%f\n" %(id, chrom, (start +(cp_indices[i] * step)), (start + (cp_indices[i+1] * step)), clogr_avg1, baf_avg1))
                # print "%s\t%s\t%d\t%d\t%f\t%f" %(id, chrom, (start +(cp_indices[i] * step)), (start + (cp_indices[i+1] * step)), clogr_avg1, baf_avg1)
                i += 1
        ...
        file.write("%s\t%s\t%d\t%d\t%f\t%f\n" %(id, chrom,(start + (cp_indices[i] * step)), (start + (cp_indices[i+1] * step)), clogr_avg2, baf_avg2))
        # print "%s\t%s\t%d\t%d\t%f\t%f" %(id, chrom,(start + (cp_indices[i] * step)), (start + (cp_indices[i+1] * step)), clogr_avg2, baf_avg2)
        ##end of merging 

    file.close()
    sys.stderr.write("coverage log ratio merging threshold: %f.\n and B-allele fraction merging threshold: %f\n" %(logr_merge_thresh, baf_merge_thresh))

def make_seg_only_logr(seg_file):
    file = open(seg_file, 'r')
    file2 = open('IGV_' + seg_file, 'w')
...
if __name__ == '__main__':
    ...
    log_ratios = calculate_logratios(read_wig(args['<tumor_coverage>']), read_wig(args['<normal_coverage>']), min_read)
    filtered_log_ratios = filter_outliers(log_ratios, 5)
    filtered_log_ratios = replace_middle_NaNs(filtered_log_ratios)
    del log_ratios

    sample_file_name = args['<tumor_coverage>'].split('.')[0]  
    allele_fractions = extract_sample(args['<B_allele_fraction>'], sample_file_name)
    smoothed_allele_fractions = smooth_allele_fractions(allele_fractions, 11)
    del allele_fractions
    
    win_size = int(args['<window_size>'])
    clogr_threshold = float(args['<clogr_threshold>'])
    baf_thresh = float(args['<BAF_threshold>'])
    logr_merge_thresh = float(args['--logr_merge'])
    baf_merge_thresh = float(args['--baf_merge'])
    z_score = int(args['--z_score'])

    win_sizes, logr_thresholds, baf_thresholds = estimate_threshold(win_size, clogr_threshold, baf_thresh, logr_merge_thresh, baf_merge_thresh, z_score)

    change_points_list = [find_consensus_change_points_win_wise(filtered_log_ratios, smoothed_allele_fractions, win_sizes[i], logr_thresholds[i], baf_thresholds[i]) for i in range(0,len(win_sizes))]       
           
    consensus_change_points = consensus(change_points_list, win_sizes, filtered_log_ratios)
    # make_wig_file(consensus_change_points, sample_file_name + '.wig')
    make_seg(filtered_log_ratios, smoothed_allele_fractions, consensus_change_points, sample_file_name + '.seg', logr_merge_thresh, baf_merge_thresh)    
